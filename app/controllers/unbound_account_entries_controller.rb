class UnboundAccountEntriesController < ApplicationController
  def new
    ...
    @account_entry = @account.unbound_account_entries.new :shareholder => @shareholder
    render :new 
  end

  def create
    @shareholder, @account = verify_account_access(current_user, params)
    @account_entry = @account.unbound_account_entries.new(params[:unbound_account_entry])
  ...
  def edit
    @shareholder, @account = verify_account_access(current_user, params)
    @account_entry = @account.unbound_account_entries.find(params[:id])
    render :edit
  end
  ...
  def update
    @shareholder, @account = verify_account_access(current_user, params)
    @account_entry = @account.unbound_account_entries.find(params[:id])
    if @account_entry.update_attributes(params[:unbound_account_entry])
