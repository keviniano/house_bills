class BalanceEvent < ActiveRecord::Base
  ...
  scope :starting_on, lambda{|start_date| where("balance_events.date >= ?", start_date)}
  scope :ending_on, lambda{|end_date| where("balance_events.date <= ?", end_date)}
  scope :with_payee_shareholder_id, lambda{|shareholder| where("(balance_events.bill_id IN (SELECT id FROM bills WHERE shareholder_id = ?) OR balance_events.account_entry_id IN (SELECT id FROM account_entries WHERE shareholder_id = ?))", shareholder, shareholder)}
  scope :with_share_shareholder_id, lambda{|shareholder| where('(balance_events.bill_id IN (SELECT bill_id FROM balance_entries WHERE shareholder_id = ?) OR balance_events.account_entry_id IN (SELECT account_entry_id FROM balance_entries WHERE shareholder_id = ?))',shareholder, shareholder)}
  scope :with_text, lambda{|text| where("balance_events.bill_id IN (SELECT id FROM bills WHERE payee ILIKE :text OR note ILIKE :text) OR balance_events.account_entry_id IN (SELECT id FROM account_entries WHERE payee ILIKE :text OR note ILIKE :text)", text: "%#{text}%")}
  ...
  scope :with_bill_type_id, lambda{|bill_type_id| where('balance_events.bill_id IN (SELECT id FROM bills WHERE bill_type_id = ?)',bill_type_id)}
